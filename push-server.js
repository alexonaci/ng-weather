...
var config = {
  publicKey: 'BDTrenhMvFL5hexEce8suYQkMeXajUwKG0NdZboLhFBM3tgJ6ENXCxv3CZxiGPDoc_1v6848KMJiaMwSkLUea8g',
  privateKey: 'oGL0vAMI0avIJCcI2gKb0VL2-_awgpqmzj6gFs4VVYc',
};

webpush.setVapidDetails('mailto:achautard@gmail.com', config.publicKey, config.privateKey);
...
function sendNotificationIfSubscriberPresent() {
  ...
  if (savedSubscriptionData) {
    console.log('send');
    webpush.sendNotification(savedSubscriptionData, JSON.stringify({ message: 'A test' })).then(function (response) {
      console.log('sent');
    });
  }
}
